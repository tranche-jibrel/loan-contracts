...
describe('JLoansForeclosedByTime', function () {
  ...
  it ("factory owner change ForeclosureWindow", async function () {
    ...
    genParams = await this.JFactory.generalParams();
    console.log("Foreclosure Window: " + genParams.foreclosureWindow);
    tx = await this.JFactory.setForeclosureWindow(50, {from: factoryOwner});
    console.log(tx.receipt.gasUsed);
    totcost = tx.receipt.gasUsed * GAS_PRICE;
    ...
    tx = await this.JFactory.setLoanGeneralParamsInContract(0, {from: factoryOwner});
    console.log(tx.receipt.gasUsed);
    genLoanParams = await this.loanContract.generalLoansParams();
  ...
  it ("foreclosure agent call foreclosed by time function", async function () {
    ...
    agentBal = await web3.eth.getBalance(foreclosureAgent);
    console.log(`foreclosureAgent Balance: ${web3.utils.fromWei(agentBal, "ether")} ETH`);
    await expectRevert(this.loanContract.setLoanToForeclosed(0, {from: foreclosureAgent}), "!Status4");
    agentBal = await web3.eth.getBalance(foreclosureAgent);
    console.log(`foreclosureAgent Balance: ${web3.utils.fromWei(agentBal, "ether")} ETH`);
    ...
    await expectRevert(this.loanContract.setLoanToForeclosed(2, {from: foreclosureAgent}), "!Status4");
    agentBal = await web3.eth.getBalance(foreclosureAgent);
    console.log(`foreclosureAgent Balance: ${web3.utils.fromWei(agentBal, "ether")} ETH`);
    ...
    console.log("Loan3 Status:" + loanStatus);
    expect(loanStatus).to.be.bignumber.equal(new BN(5).toString());
    console.log("Loan0 Foreclosed Block:" + await this.loanContract.loanForeclosedBlock(0));
    console.log("Loan1 Foreclosed Block:" + await this.loanContract.loanForeclosedBlock(1));
    console.log("Loan2 Foreclosed Block:" + await this.loanContract.loanForeclosedBlock(2));
    console.log("Loan3 Foreclosed Block:" + await this.loanContract.loanForeclosedBlock(3));
  });
  ...
  it('borrowers cannot send any collateral to foreclosed contract', async function () {
    ...
    await expectRevert(this.loanContract.depositEthCollateral(1, {from: borrower2, value: one_eth}), "!Status14");
  });
